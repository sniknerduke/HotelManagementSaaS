            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-x-12">
                ...
                <div className="lg:col-span-7 space-y-16">
                    <h1 className="text-4xl md:text-5xl font-serif text-[#1A1A1A] mb-12">
                        Complete your <span className="italic text-[#D4AF37]">reservation.</span>
                    </h1>
                    ...
                    <div className="flex items-center gap-4 text-[10px] uppercase tracking-[0.2em] font-medium text-[#6C6863] border-b border-[#1A1A1A]/10 pb-6 mb-12">
                         <span className={step >= 1 ? "text-[#1A1A1A]" : ""}>01 Details</span>
                         <span className="w-12 h-px bg-[#1A1A1A]/20"></span>
                         <span className={step >= 2 ? "text-[#1A1A1A]" : ""}>02 Payment</span>
                         <span className="w-12 h-px bg-[#1A1A1A]/20 hidden sm:block"></span>
                         <span className={step >= 3 ? "text-[#1A1A1A] hidden sm:block" : "hidden sm:block"}>03 Confirmation</span>
                    </div>

                    {step === 1 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                            <h2 className="text-2xl font-serif text-[#1A1A1A] mb-8 border-b border-[#1A1A1A]/10 pb-4">Guest Information</h2>
                            ...
                            <div className="pt-8 flex justify-end">
                                <Button onClick={handleNext} className="w-full sm:w-auto">Continue to Payment</Button>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 2 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                            <h2 className="text-2xl font-serif text-[#1A1A1A] mb-8 border-b border-[#1A1A1A]/10 pb-4">Payment Details</h2>
                            ...
                            <div className="pt-8 flex justify-between">
                                <Button variant="ghost" onClick={handlePrev} className="hidden sm:flex text-[#6C6863] hover:text-[#1A1A1A]">Back to Details</Button>
                                <Button onClick={handleNext} className="w-full sm:w-auto bg-[#D4AF37] hover:bg-[#1A1A1A] text-white">Confirm Reservation</Button>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 3 && (
                        <div className="text-center py-16 space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-[1000ms]">
                            <p className="text-[10px] uppercase font-bold tracking-[0.3em] text-[#D4AF37] mb-6">Reservation Confirmed</p>
                            <h2 className="text-4xl lg:text-6xl font-serif text-[#1A1A1A] leading-[1.1]">
                                Thank you for<br/><span className="italic text-[#6C6863]">choosing Lumière.</span>
                            </h2>
                            <p className="text-[#6C6863] pt-6 font-serif italic">Your booking reference is <strong className="font-sans not-italic text-[#1A1A1A]">#RES-10492</strong></p>
                            <div className="pt-12">
                                <Button onClick={() => window.location.href = '/profile'} variant="secondary">View My Stays</Button>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="lg:col-span-4 lg:col-start-9 md:sticky md:top-36 h-max">
                    <Card variant="featured" className="py-8 bg-white/80 backdrop-blur-sm -mx-4 md:mx-0 shadow-[0_8px_32px_rgba(0,0,0,0.06)] border-t-[6px]">
                         <h3 className="text-[10px] uppercase font-bold tracking-[0.3em] text-[#1A1A1A] mb-8 pb-4 border-b border-[#1A1A1A]/10">Your Stay</h3>
